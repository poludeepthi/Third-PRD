CampusTrack: Student Academic Management 
...
Select the course using a menu.
Read and validate five subject marks.
Calculate total, percentage, subject result and grade.
Read attendance data and calculate the attendance percentage.
Process assignment scores.
Calculate the scholarship discount.
Read the fee amount paid and calculate the balance.
Determine the final semester-clearance status.
Display failed conditions and recommendations.
...
Assignment rules
If the score is -1, stop the assignment loop using break.
If the score is below -1 or above 10, display a warning and skip the entry using continue.
A valid score from 0 to 10 must be added to the assignment total.
Increase the valid-assignment count only for an accepted score.
Invalid and skipped entries must not affect the total or average.
If no valid score is entered, the assignment average must be 0.00.
Calculate:
...
Attendance rules
Assignment rules
Scholarship and fee rules
Final-clearance rules
Pseudocode
Test cases
Sample input and output
Screenshots or copied console outputs of completed tests
...
Create a public GitHub repository named Third-PRD.
Follow the required repository structure exactly.
Write the complete program in src/Main.java.
Write the pseudocode in pseudocode/pseudocode.txt before completing the Java program.
Execute at least five test cases.
Copy each test result into a separate file inside the output folder.
Update the project README.md with the completed project details.
Verify that the program compiles and runs without errors.
Submit the GitHub repository URL using the instructed submission channel.
...
The final application should demonstrate that the learner can collect data, validate input, apply business rules, perform calculations, control program flow and generate a meaningful real-wor
